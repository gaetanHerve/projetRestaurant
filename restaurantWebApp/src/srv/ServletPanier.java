...
//		session.setAttribute("client", client);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO : eviter de récupérer le panier et la carte ici et impacter ceux du doGet();
		HttpSession session = request.getSession();
		ServletContext app = request.getServletContext();
		Panier panier = (Panier)session.getAttribute("panier");
		System.out.println(panier);
		
		ArrayList<LignePanier> lignes = (panier.getLignes() != null)
				? new ArrayList<LignePanier>(panier.getLignes())
				: new ArrayList<LignePanier>();
		
		Carte carte = (Carte)app.getAttribute("carte");
		int qte = Integer.parseInt(request.getParameter("quantity"));
